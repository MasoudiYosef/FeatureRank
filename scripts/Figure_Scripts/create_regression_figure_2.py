...
def find_nearest_feature_file(
    root: Path, requested_ratio: str, pattern: str
) -> Path | None:
...
def find_val_convergence_csv(base_dir: Path, config: DatasetConfig) -> Path | None:
    ...
    if matches:
        return matches[0]
    fallback = find_nearest_feature_file(
        root, config.feature_ratio, "top_*_average_convergence.csv"
    )
...
def find_val_convergence_image(base_dir: Path, config: DatasetConfig) -> Path | None:
    ...
    if matches:
        return matches[0]
    return find_nearest_feature_file(
        root, config.feature_ratio, "top_*_average_error_convergence.png"
    )


def regression_runs_path(base_dir: Path, config: DatasetConfig) -> Path:
    return (
        metrics_dir(base_dir, config)
        / f"top_{config.feature_ratio}_regression_runs.csv"
    )


def metric_runs_text_path(base_dir: Path, config: DatasetConfig) -> Path:
    return (
        metrics_dir(base_dir, config) / f"top_{config.feature_ratio}_pearson_r_runs.txt"
    )
...
def table_summary_path(base_dir: Path, config: DatasetConfig) -> Path:
    return (
        metrics_dir(base_dir, config)
        / f"top_{config.feature_ratio}_regression_table_summary.json"
    )


def test_metrics_path(base_dir: Path, config: DatasetConfig) -> Path:
    return (
        metrics_dir(base_dir, config) / f"top_{config.feature_ratio}_test_metrics.json"
    )
...
def load_prediction_errors(
    ...
    for split in ("train", "test"):
        ...
        if not path.exists() and split == "test":
            legacy_path = (
                dataset_dir(base_dir, config)
                / f"top_{config.feature_ratio}_prediction_errors.csv"
            )
            path = legacy_path
        if not path.exists():
            if split == "train":
                ...
                print(
                    f"[WARN] Test prediction errors missing for {config.label}: {path}"
                )
            continue
        df = pd.read_csv(path)
...
def drop_worst_error_fraction(
    df: pd.DataFrame, fraction: float = WORST_ERROR_DROP_FRACTION
) -> tuple[pd.DataFrame, bool]:
    if df.empty or fraction <= 0 or len(df) < MIN_POINTS_FOR_WORST_ERROR_DROP:
        return df, False
    keep_count = max(1, int(np.ceil(len(df) * (1.0 - fraction))))
    return (
        df.sort_values("absolute_error", ascending=True).head(keep_count).sort_index(),
        True,
    )
...
def plot_actual_predicted_panel(
    ...
    if np.isclose(value_range, 0.0):
        ...
        df["true_value_norm"] = 0.5 + 0.5 * (
            (df["true_value"].astype(float) - min_value) / value_range
        )
        df["predicted_value_norm"] = 0.5 + 0.5 * (
            (df["predicted_value"].astype(float) - min_value) / value_range
        )

    split_styles = {
        "train": {
            "marker": "o",
            "facecolors": config.point_color,
            "edgecolors": "black",
            "label": "Train",
        },
        "test": {
...
def load_rmse_runs(base_dir: Path, config: DatasetConfig) -> list[float]:
    ...
    text_runs = load_metric_list_from_text(
        metric_runs_text_path(base_dir, config), "RMSE listesi"
    )
...
def plot_rmse_boxplot_panel(
    ax, configs: list[DatasetConfig], base_dir: Path, panel: str = "D"
) -> None:
...
def plot_convergence_panel(
    ax, configs: list[DatasetConfig], base_dir: Path, panel: str = "E"
) -> None:
    plotted = False
    y_label = "Convergence"
    for config in configs:
...
def load_correlation_value(base_dir: Path, config: DatasetConfig) -> float | None:
    for path in [
        table_summary_path(base_dir, config),
        test_metrics_path(base_dir, config),
    ]:
        if not path.exists():
            continue
...
def plot_correlation_panel(
    ax, configs: list[DatasetConfig], base_dir: Path, panel: str = "F"
) -> None:
    rows = []
    for config in configs:
    ...
    bars = ax.bar(
        labels, values, color="black", edgecolor="black", linewidth=1.2, alpha=1.0
    )
    ...
    ax.set_ylabel(
        "Pearson correlation coefficient", fontsize=AXIS_LABEL_FONTSIZE, color="black"
    )
    ax.set_ylim(0.0, 1.0)
...
def create_regression_figure_2(
    configs: list[DatasetConfig], base_dir: Path, output_path: Path, dpi: int
) -> None:
    ...
    fig.tight_layout()
    fig.subplots_adjust(
        left=0.055, right=0.99, top=0.93, bottom=0.1, wspace=0.22, hspace=0.35
    )
...
def main() -> None:
    parser = argparse.ArgumentParser(
        description="Create a six-panel regression Figure 2 from saved experiment outputs."
    )
    parser.add_argument(
        "--items",
        nargs="+",
    ...
    parser.add_argument("--base-dir", type=str, default="outputs/Regression")
    parser.add_argument(
        "--output",
        type=str,
        default="outputs/Regression/figure_2_regression_results.jpeg",
    )
    parser.add_argument("--dpi", type=int, default=300)
    args = parser.parse_args()

    configs = (
        [parse_item(item) for item in args.items]
        if args.items
        else load_configs_from_defaults()
    )
    if len(configs) < 3:
        raise ValueError(
            "Figure 2 needs at least three dataset configs for panels A, B and C."
        )
