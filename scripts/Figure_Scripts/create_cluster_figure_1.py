...
def build_cluster_colormap(cluster_count: int) -> ListedColormap:
    if cluster_count <= PCA_CLUSTER_CMAP.N:
        return ListedColormap(
            PCA_CLUSTER_CMAP.colors[:cluster_count], name="feature_rank_cluster_dynamic"
        )
    return ListedColormap(
...
def build_source_path(
    config: ClusterFigureConfig, k_value: int, plot_kind: str
) -> Path | None:
...
def trim_white_border(image: np.ndarray, padding: int = 6) -> np.ndarray:
    ...
    non_white_mask = (
        np.any(rgb_image < 0.985, axis=-1) if rgb_image.ndim == 3 else rgb_image < 0.985
    )
...
def draw_panel_b_pca(
    ...
    if pca_data_path is None or not pca_data_path.exists():
        draw_image_or_placeholder(
            ax, image_path, "", image_aspect="equal", trim_border=True
        )
        return None, 0
    ...
    pc1_variance = (
        float(pca_df["pc1_variance"].iloc[0])
        if "pc1_variance" in pca_df.columns
        else 16.4
    )
    pc2_variance = (
        float(pca_df["pc2_variance"].iloc[0])
        if "pc2_variance" in pca_df.columns
        else 8.0
    )
...
def draw_panel_a_elbow_silhouette(ax, config: ClusterFigureConfig) -> None:
    ...
    ax_inertia.set_ylabel(
        "WCSS", color="#1f77b4", fontsize=AXIS_LABEL_FONTSIZE, labelpad=8
    )
    ...
    ax_inertia.tick_params(
        axis="y", labelsize=TICK_LABEL_FONTSIZE, labelcolor="#1f77b4"
    )
    ...
    ax_silhouette.set_ylabel(
        "Silhouette score", color="#ff7f0e", fontsize=AXIS_LABEL_FONTSIZE
    )
    ax_silhouette.tick_params(
        axis="y", labelsize=TICK_LABEL_FONTSIZE, labelcolor="#ff7f0e"
    )
...
def create_cluster_figure(config: ClusterFigureConfig) -> list[Path]:
    ...
    panel_b_axes = [
        fig.add_subplot(grid[1, col_idx]) for col_idx in range(len(config.k_values))
    ]
    ...
    for col_idx, k_value in enumerate(config.k_values):
        ...
        pca_data_path = (
            build_pca_data_path(config, k_value) if config.use_pca_csv else None
        )
    ...
    fig.subplots_adjust(
        left=0.05, right=0.97, top=0.92, bottom=0.055, wspace=0.04, hspace=0.07
    )
    ...
    for colorbar_index, (ax, scatter, cluster_count) in enumerate(pca_colorbars):
        ...
        colorbar_ax = fig.add_axes(
            [position.x1 + 0.008, position.y0, 0.011, position.height]
        )
        colorbar = fig.colorbar(
            scatter, cax=colorbar_ax, ticks=np.arange(cluster_count)
        )
...
def parse_args() -> ClusterFigureConfig:
    parser = argparse.ArgumentParser(
        description="Create publication-ready cluster Figure 1 from saved clustering plots."
    )
    parser.add_argument(
        "--dataset-name",
        default=DEFAULT_DATASET_NAME,
        help="Dataset folder name under outputs/Clustering",
    )
    parser.add_argument(
        "--top-percentage",
        type=int,
        default=DEFAULT_TOP_PERCENTAGE,
        help="Feature selection percentage, e.g. 20, 40, 60",
    )
    parser.add_argument(
        "--k-values",
        type=int,
        ...
        help="K values to display, e.g. 2 8 10",
    )
    parser.add_argument(
        "--panel-a-k",
        type=int,
        default=DEFAULT_PANEL_A_K,
        help="K value used for the single Panel A elbow/silhouette image",
    )
    parser.add_argument(
        "--base-dir",
        type=Path,
        default=DEFAULT_BASE_DIR,
        help="Base clustering output directory",
    )
    parser.add_argument(
        "--output-dir",
        type=Path,
        default=DEFAULT_OUTPUT_DIR,
        help="Directory for final figure outputs",
    )
    parser.add_argument(
        "--output-prefix", default=None, help="Output filename without extension"
    )
    parser.add_argument("--dpi", type=int, default=300, help="Output DPI")
    parser.add_argument(
        "--use-pca-csv",
        action="store_true",
        help="Redraw PCA panels from saved CSV data instead of embedding saved PNG images",
    )
