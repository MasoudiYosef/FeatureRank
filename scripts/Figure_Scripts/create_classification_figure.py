...
def prediction_path(base_dir: Path, config: DatasetConfig) -> Path:
    return (
        base_dir
        / config.dataset_name
        / f"top_{config.feature_ratio}_classification_predictions.csv"
    )
...
def accuracy_runs_path(base_dir: Path, config: DatasetConfig) -> Path:
    return (
        ...
        / config.dataset_name
        / "metrics"
        / f"top_{config.feature_ratio}_accuracy_runs.txt"
    )
...
def load_prediction_data(base_dir: Path, config: DatasetConfig) -> pd.DataFrame | None:
    ...
    if missing_columns:
        print(
            f"[WARN] Prediction file missing columns: {path} -> {sorted(missing_columns)}"
        )
        return None
    return df
...
def plot_aggregate_confusion_matrix(
    ax, prediction_frames: list[tuple[DatasetConfig, pd.DataFrame]]
) -> None:
...
def plot_accuracy_boxplots(
    ax, accuracy_data: list[tuple[DatasetConfig, list[float]]]
) -> None:
    labels: list[str] = []
...
def create_classification_figure(
    ...
    if not prediction_frames:
        raise ValueError(
            "No prediction CSV files found. Run binary classification first."
        )
    ...
    prediction_frames = sorted(
        prediction_frames,
        key=lambda item: accuracy_mean_by_dataset.get(
            item[0].dataset_name, float("-inf")
        ),
        reverse=True,
    )
    ...
    fig.subplots_adjust(
        left=0.055, right=0.985, top=0.905, bottom=0.08, wspace=0.30, hspace=0.40
    )
...
def main() -> None:
    ...
    parser.add_argument("--base-dir", type=str, default="outputs/Classification")
    parser.add_argument(
        "--output",
        type=str,
        default="outputs/Classification/classification_summary_figure.jpeg",
    )
    ...
    parser.add_argument(
        ...
        type=str,
        default="",
        help="D panelindeki tum box/kutu renklerini bu renge ayarlar.",
    )
    parser.add_argument(
        "--point-color",
        type=str,
        default="",
        help="D panelindeki tum yuvarlak nokta renklerini bu renge ayarlar.",
    )
