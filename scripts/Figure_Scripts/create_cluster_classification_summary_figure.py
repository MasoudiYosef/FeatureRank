...
#
# dataset_name:
#   Folder name under outputs/Clustering or outputs/Classification.
# label:
...
#   outputs/Classification/<dataset>/metrics/top_<feature_ratio>_test_metrics.json.
    ...
    for item in raw_configs:
        ...
        configs.append(
            DatasetConfig(
                ...
                true_class_count=(
                    int(item["true_class_count"])
                    if item.get("true_class_count") is not None
                    else None
                ),
...
def cluster_scores_path(config: DatasetConfig, clustering_base_dir: Path) -> Path:
    return (
        clustering_base_dir
        / config.dataset_name
        / f"top_{config.feature_ratio}_cluster_scores.csv"
    )


def classification_metrics_path(
    config: DatasetConfig, autoencoder_base_dir: Path
) -> Path:
    return (
        autoencoder_base_dir
        / config.dataset_name
        / "metrics"
        / f"top_{config.feature_ratio}_test_metrics.json"
    )
...
def read_optimum_k_values(
    ...
    if not path.exists():
        print(
            f"[WARN] Cluster score file not found: {path}. Optimum k skipped for {config.label}."
        )
        return []
    ...
    if not required_columns.issubset(scores_df.columns):
        print(
            f"[WARN] Cluster score file missing columns: {path}. Required: {sorted(required_columns)}"
        )
        return []

    valid_scores = scores_df.dropna(subset=["silhouette_score"]).copy()
    valid_scores = valid_scores[
        (valid_scores["k"] >= K_MIN) & (valid_scores["k"] <= K_MAX)
    ]
    if valid_scores.empty:
        print(f"[WARN] No valid silhouette scores in k={K_MIN}-{K_MAX}: {path}")
        return []

    best_rows = valid_scores.sort_values("silhouette_score", ascending=False).head(
        auto_optimum_count
    )
...
def read_classification_metrics(
    ...
    if not path.exists():
        print(
            f"[WARN] Classification metric file not found: {path}. Using NaN values for {config.label}."
        )
...
def plot_panel_a(
    ax, configs: list[DatasetConfig], optimum_k_map: dict[str, list[int]]
) -> None:
...
def plot_panel_b(
    ax, configs: list[DatasetConfig], metrics_map: dict[str, dict[str, float]]
) -> None:
    ...
    for x_idx, config in enumerate(configs):
        ...
        sorted_dataset_metrics = sorted(
            ...
            key=lambda item: (
                float(item[1])
                if item[1] is not None and not np.isnan(float(item[1]))
                else -np.inf
            ),
            reverse=True,
        )
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Create clustering/classification summary figure."
    )
    parser.add_argument(
        "--clustering-base-dir", type=Path, default=Path("outputs") / "Clustering"
    )
    parser.add_argument(
        "--autoencoder-base-dir", type=Path, default=Path("outputs") / "Classification"
    )
    parser.add_argument("--output-dir", type=Path, default=OUTPUT_DIR)
    parser.add_argument("--output-basename", type=str, default=OUTPUT_BASENAME)
    parser.add_argument(
        "--no-show", action="store_true", help="Do not show the figure after saving."
    )
