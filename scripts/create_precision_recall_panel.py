...
def parse_item(item_text: str) -> tuple[str, str, str]:
	...
	if len(parts) not in {2, 3}:
		raise ValueError("Her item 'dataset_folder:percent[:label]' formatinda olmali. Ornek: pid_data:80:PID")
...
def load_predictions(dataset_folder: str, feature_percent: str) -> pd.DataFrame:
	...
	if missing_columns:
		raise ValueError(f"Prediction dosyasinda eksik kolon var: {prediction_path}, {sorted(missing_columns)}")
	return predictions_df


def main() -> None:
	parser = argparse.ArgumentParser(description="Binary classification precision-recall paneli olusturur.")
	parser.add_argument(
		"--items",
		nargs="+",
		required=True,
	...
	parser.add_argument(
		"--output",
		type=str,
		default="outputs/autoencoder/classification_precision_recall_panel.png",
	...
	for item_text in args.items:
		...
		plt.plot(recall, precision, linewidth=1.6, label=f"{label} (AP={average_precision:.3f})")
