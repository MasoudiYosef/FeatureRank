...
def load_labels(path: Path) -> np.ndarray:
    return pd.read_csv(
        path,
        header=None,
    ).iloc[:, 0].to_numpy(dtype=np.int32)
...
def parse_dropout_rates(
    ...
    if len(values) != hidden_count:
        raise ValueError(
            "classifier-dropout-rates uzunlugu hidden layer sayisiyla ayni olmali."
        )
    return values
...
def direct_binary_accuracy(
    ...
    accuracy, _, _ = ra.train_and_evaluate_direct_classifier(
        ...
        classifier_hidden_units=args.classifier_hidden_units_parsed,
        classifier_dropout_rates=args.classifier_dropout_rates_parsed,
...
def evaluate_classification(
    ...
    class_counts = {
        int(label): int(
            np.sum(
                np.concatenate([y_train, y_test]) == label
            )
        )
    ...
    for class_index, class_label in enumerate(class_labels):
        ...
        if history_root is not None:
            class_history_dir = (
                history_root / f"class_{class_label}"
            )
        ...
        class_accuracy = direct_binary_accuracy(
            ...
            if class_history_dir is not None
            else None,
        )
    ...
    total_weight = float(
        sum(row["class_count"] for row in class_rows)
    )
    weighted_accuracy = float(
        sum(
            row["accuracy"] * row["class_count"]
...
def main():
    args = parse_args()
    args.classifier_hidden_units_parsed = parse_hidden_units(
        args.classifier_hidden_units
    )
    ...
    metadata_files = sorted(
        output_root.glob(
            "reduction_*_retained_*/run_*/metadata.json"
        )
    )
    if not metadata_files:
        raise FileNotFoundError(
            f"metadata.json bulunamadi: {output_root}"
        )
    ...
    for exp_idx, metadata_path in enumerate(
        ...
        X_train = load_matrix(
            Path(metadata["train_data_path"])
        )
        y_train = load_labels(
            Path(metadata["train_label_path"])
        )
        X_test = load_matrix(
            Path(metadata["test_data_path"])
        )
        y_test = load_labels(
            Path(metadata["test_label_path"])
        )
        ...
        row = {
            ...
            "retained_percent": int(
                metadata["retained_percent"]
            ),
            "original_feature_count": int(
                metadata["original_feature_count"]
            ),
            "encoding_layer_neurons": int(
                metadata["encoding_layer_neurons"]
            ),
            "accuracy": float(accuracy),
            "rmse": float(
                metadata["reconstruction_rmse"]
            ),
    ...
    all_runs_path = (
        output_root
        / "dimension_reduction_all_runs.csv"
    )
    all_runs_df.to_csv(
        all_runs_path,
        index=False,
    ...
    results_path = (
        output_root
        / "dimension_reduction_results.csv"
    )
    summary_df.to_csv(
        results_path,
        index=False,
