...
ID_COLUMN = "none"
OUTPUT_DIRECTORY = "split_datasets"
...
def load_dataset_with_target(
    ...
    """
    Reads either a normal CSV containing the target column or this project's
    raw *_data.csv + *_label.csv pair through load_data().
    """
...
def split_feature_columns_evenly(feature_columns: list[str], number_of_blocks: int) -> list[list[str]]:
...
def write_run_autoencoder_pair(
...
def write_named_run_autoencoder_pair(
    df: pd.DataFrame,
    feature_columns: list[str],
    target_column: str,
    output_stem: str,
...
def split_dataset_by_features(
    ...
    write_run_autoencoder_files: bool = True,
    run_autoencoder_raw_dir: str | Path = PROJECT_ROOT / "data" / "raw",
) -> dict:
    loaded = load_dataset_with_target(
        input_file=input_file,
        target_column=target_column,
        id_column=normalize_optional_text(id_column),
    )
    df = loaded.df
    ...
    blocks = split_feature_columns_evenly(feature_columns, number_of_blocks)
    summary_rows: list[dict] = []
    mapping_rows: list[dict] = []

    raw_dir = Path(run_autoencoder_raw_dir)
    if write_run_autoencoder_files:
        ensure_dir(raw_dir)

    original_feature_lookup = {feature_name: idx + 1 for idx, feature_name in enumerate(feature_columns)}
    sample_count = len(df)
    ...
    for block_number, block_features in enumerate(blocks, start=1):
        ...
        if write_run_autoencoder_files:
            pair_data_path, pair_label_path = write_run_autoencoder_pair(
                block_df=block_df,
        ...
        if write_run_autoencoder_files:
            print(f"Run autoencoder data : {pair_data_path}")
            print(f"Run autoencoder label: {pair_label_path}")
    ...
    duplicate_features = mapping_df["Feature_Name"][mapping_df["Feature_Name"].duplicated()].tolist()
...
def collect_selected_features(selected_features_dir: str | Path, selected_features_glob: str) -> pd.DataFrame:
    ...
    if not paths:
        ...
        raise FileNotFoundError(f"Secilmis feature dosyasi bulunamadi. Denenen patternler:\n{tried}")
...
def combine_selected_features(
    ...
    selected_df = collect_selected_features(
        ...
        selected_features_glob=selected_features_glob,
    )
    ...
    if write_run_autoencoder_files:
        raw_dir = Path(run_autoencoder_raw_dir)
        ensure_dir(raw_dir)
        pair_data_path, pair_label_path = write_named_run_autoencoder_pair(
            df=final_df,
    ...
    if mapping_file:
        ...
        if mapping_path.exists():
            ...
            selected_mapping.to_csv(output_path.parent / f"{output_path.stem}_selected_feature_mapping.csv", index=False)
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--mode", choices=["split", "combine", "split-and-combine"], default="split")
    ...
    parser.add_argument("--write-run-autoencoder-files", action=argparse.BooleanOptionalAction, default=True)
    parser.add_argument("--run-autoencoder-raw-dir", default=str(PROJECT_ROOT / "data" / "raw"))
...
def main() -> None:
    ...
    if selected_glob is None:
        percent_tag = int(args.selection_percentage) if float(args.selection_percentage).is_integer() else args.selection_percentage
    ...
    if args.mode in {"split", "split-and-combine"}:
        split_result = split_dataset_by_features(
            ...
            write_run_autoencoder_files=args.write_run_autoencoder_files,
            run_autoencoder_raw_dir=args.run_autoencoder_raw_dir,
        )
