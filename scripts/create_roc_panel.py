...
def main() -> None:
	parser = argparse.ArgumentParser(description="Binary classification ROC paneli olusturur.")
	parser.add_argument(
		"--items",
		nargs="+",
		required=True,
	...
	parser.add_argument(
		"--output",
		type=str,
		default="outputs/autoencoder/classification_roc_panel.png",
	...
	for item_text in args.items:
		...
		plt.plot(fpr, tpr, linewidth=1.6, label=f"{label} (AUC={roc_auc:.3f})")
	...
	plt.close()
	print(f"[OK] ROC panel: {output_path}")
